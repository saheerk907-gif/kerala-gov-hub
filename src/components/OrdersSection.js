    ...
    <section id="orders" className="relative py-8">
      <div>
        ...
        <div className="relative mb-4">
          ...
          />
          {query && (
            <button onClick={() => handleSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/50 hover:text-white/60 transition-colors bg-transparent border-0 cursor-pointer text-lg leading-none">
              ×
            </button>
          )}
        </div>
        ...
        <div>
          {currentOrders.length === 0 ? (
            <div className="text-center py-10 text-white/50 text-[13px]" style={{ fontFamily: "var(--font-noto-malayalam), sans-serif" }}>
              ...
              <div key={o.id} className="glass-card relative flex items-center gap-4 p-4 md:p-5 rounded-[20px] mb-3 opacity-40 cursor-not-allowed">
                <div className="flex-shrink-0 w-11 h-11 rounded-xl flex items-center justify-center text-lg bg-white/[0.06] text-white/50">
                  📄
                </div>
                <div className="flex-grow min-w-0">
                  <div className="flex items-center gap-2 mb-1">
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-1.5 mt-4 flex-wrap">
            <button onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1}
              className="px-4 py-3 rounded-lg text-[12px] font-semibold text-white/50 disabled:opacity-20 disabled:cursor-not-allowed hover:text-white hover:bg-white/[0.06] transition-all border-0 bg-transparent cursor-pointer">
              ← മുമ്പ്
            </button>
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(page => {
              ...
              if (showEllipsisBefore || showEllipsisAfter)
                return <span key={`e-${page}`} className="text-white/45 px-1 text-sm">…</span>;
              if (!show) return null;
              return (
