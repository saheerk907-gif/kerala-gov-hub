...
function SmallCard({ article }) {
  ...
  return (
    <Link href={`/articles/${article.id}`} className="no-underline group flex flex-col">
      <div className="rounded-2xl overflow-hidden flex flex-col h-full transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_12px_40px_rgba(0,0,0,0.4)]"
        style={{ background: '#0d0d0d', border: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="relative overflow-hidden" style={{ height: '140px', background: '#111', flexShrink: 0 }}>
          {article.image_url ? (
            <img src={article.image_url} alt={article.title_ml}
        ...
        <div className="flex flex-col flex-1 p-4">
          <div className="text-[10px] text-white/25 mb-1.5">{formatDate(article.created_at)}</div>
          <h3 className="text-[14px] font-bold text-white/90 leading-snug group-hover:text-white transition-colors line-clamp-3"
            style={{ fontFamily: "'Meera', sans-serif" }}>
            {article.title_ml}
          </h3>
          <div className="mt-3 flex items-center gap-1 text-[11px] font-bold transition-colors" style={{ color }}>
            വായിക്കുക
...
export default function ArticlesSection() {
  ...
  return (
    <section className="py-14 px-4 md:px-6 max-w-[1400px] mx-auto">
      {/* Header */}
