    ...
    <>
      ...
      <main className="min-h-screen bg-aurora text-white pt-24 pb-16 px-4 md:px-6">
        <div className="max-w-3xl mx-auto">
          ...
          {/* Header */}
          <div className="mb-10">
            <Link href="/" className="text-[12px] text-white/35 hover:text-white no-underline transition-colors">
              ← keralagovhub.in
            </Link>
          ...
          {loading ? (
            ...
            <div className="glass-card rounded-2xl p-12 text-center text-white/35">
              വാർത്തകൾ ലഭ്യമല്ല
            </div>
          ) : (
            <div className="flex flex-col gap-2">
              {news.map((item, i) => (
                <Link key={item.id} href={`/news/${item.id}`}
                  ...
                  {/* Index */}
                  <div className="text-[11px] font-black text-white/20 w-6 flex-shrink-0 text-right">{i + 1}</div>
                  ...
                  <svg className="w-4 h-4 text-white/20 group-hover:text-white/60 group-hover:translate-x-0.5 transition-all flex-shrink-0"
