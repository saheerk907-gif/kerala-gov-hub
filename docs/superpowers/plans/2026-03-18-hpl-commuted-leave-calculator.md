# HPL & Commuted Leave Calculator Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an HPL & Commuted Leave Balance Estimator component to the existing `/leave` page of the Kerala government employee portal.

**Architecture:** A single new client component `HPLCalculator.js` handles all state, calculation logic, and rendering. Pure helper functions are defined at the top of the file. The component is mounted below the existing `<LeaveCalculator />` in `src/app/leave/page.js`. No Supabase, no new routes.

**Tech Stack:** Next.js 14 App Router, React 18 (`useState`, `useMemo`, `useEffect`, `useRef`), Tailwind CSS, existing `glass-card` utility class, `SectionHeader` component.

**Spec:** `docs/superpowers/specs/2026-03-18-hpl-commuted-leave-calculator-design.md`

---

## Chunk 1: HPLCalculator Component

### Task 1: Pure calculation helpers

**Files:**
- Create: `src/components/HPLCalculator.js` (helpers only for now — component shell added in Task 2)

**Context:** All calculation logic lives as plain functions at the top of the file, above the React component. This makes them easy to reason about independently.

- [ ] **Step 1: Create the file with helpers**

Create `src/components/HPLCalculator.js` with this exact content:
 ...
 * exclusive day count excluded from service. Both from and to are inclusive.
...
Open `src/components/HPLCalculator.js` and confirm:
- `countFullAnniversaries` handles the boundary (≤ comparison with `candidate <= end`)
- `calcExcludedDays` merges overlapping ranges and clamps to today / joiningDate
- `calculateHPL` returns all 9 fields
- `checkLeave` returns `null` for `desired <= 0`

---

### Task 2: Animated counter hook

**Files:**
- Modify: `src/components/HPLCalculator.js` — append hook after the helpers

**Context:** A small `useAnimatedCounter` hook drives all 5 number counters in the results panel. Re-triggers on value change.

- [ ] **Step 1: Append the hook to HPLCalculator.js**

Add this block immediately after the `checkLeave` function (before the component export):
...
Confirm the hook is between `checkLeave` and the component export (which will be written in Task 3).
...
**Files:**
- Modify: `src/components/HPLCalculator.js` — append the full React component

**Context:** The component follows the exact same glass-card pattern as `LeaveCalculator.js`. Accent colour is `#64d2ff` (blue). `SectionHeader` and `inputCls`/`labelCls` match the existing file exactly.

- [ ] **Step 1: Append the component to HPLCalculator.js**

Add this block at the end of the file (after the hook):
...
2. Date helpers (`todayStr`, `countFullAnniversaries`, `calcExcludedDays`)
3. `calculateHPL`
4. `checkLeave`
5. `useAnimatedCounter`
6. Style constants + `StatCard` + `EligibilityBadge`
7. `export default function HPLCalculator`

---

## Chunk 2: Wire into Leave Page + ToolsSection Update

### Task 4: Add HPLCalculator to /leave page

**Files:**
- Modify: `src/app/leave/page.js`

**Context:** The page already imports and renders `<LeaveCalculator />`. We add `<HPLCalculator />` directly below it inside the same `<div>`.

- [ ] **Step 1: Edit leave/page.js**

In `src/app/leave/page.js`:

1. Add import at the top (after the LeaveCalculator import):
```js
import HPLCalculator from '@/components/HPLCalculator';
```

2. In the JSX, after `<LeaveCalculator />`, add:
```jsx
<HPLCalculator />
```

The full `<main>` block should look like:
```jsx
...
- [ ] **Step 2: Update page metadata description**

Update the `description` in the metadata object in `leave/page.js` to mention HPL and Commuted Leave:
...
### Task 5: Update ToolsSection card description

**Files:**
- Modify: `src/components/ToolsSection.js`

**Context:** The Leave Calculator card currently says "EL, CL, ML, Study Leave balance and encashment estimate". Update it to reflect the new HPL/Commuted Leave features.

- [ ] **Step 1: Update the tools array entry**

In `src/components/ToolsSection.js`, find the Leave Calculator entry and update `desc` and `tags`:
...
```bash
cd "/home/saheer-anas-k/kerala-gov-hub-main " && npm run dev
# Note: the project directory has a trailing space in the name — include it exactly as shown
```

- [ ] **Step 2: Manual smoke test checklist**

Navigate to `http://localhost:3000/leave` and verify:

1. Page loads without console errors
2. Existing EL calculator renders above the divider
3. "Half-Pay & Commuted Leave" divider appears
4. HPL header card shows with blue accent
5. Employment Type toggle works (Permanent ↔ Non-Permanent)
6. Enter a Date of Joining → results panel appears with animated counters
7. Non-permanent + < 3 years service → Commuted Leave shows as ineligible (amber badge)
8. Non-permanent + ≥ 3 years service → Eligible (green badge)
9. Add an LWA period → excluded days shown, completedYears changes correctly
10. HPL taken + Commuted taken exceeding total → overflow warning shown
11. CL taken > 20 → warning shown
12. "Can I Take X Days?" — test HPL, Commuted, and CL cases (yes + no)
13. KSR reference table renders at bottom
14. ToolsSection on home page shows updated tags

- [ ] **Step 3: Commit**

```bash
git add "src/components/HPLCalculator.js" "src/app/leave/page.js" "src/components/ToolsSection.js"
git commit -m "feat: add HPL & Commuted Leave calculator to /leave page

- New HPLCalculator component with animated counters and progress bars
- Anniversary-based completed years calculation (handles leap years)
- LWA exclusion periods with overlap merging
- 'Can I take X days?' eligibility checker for HPL, Commuted, and CL
- Commuted Leave eligibility badge with pulse styling
- KSR rules quick-reference table
- Updated /leave page metadata and ToolsSection description
